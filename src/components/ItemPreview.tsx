  ...
  const handleLocationChange = (
    ...
    if (event.target) {
      ...
      getLocation(item.gid, target.value)
        .then((response) =>
          setItemData({ ...itemData, location: response.data }),
        )
        .catch((e) => console.log(e));
    }
  };
      ...
      changeItemFields(newData, item.gid, item.iid)
        ...
        .then((response) => {
          if (itemData.location.lid === location.lid) {
            dispatch(
              setLocationItems(
                locationItems.map((locItem) => {
                  return locItem.iid === response.data.iid
                    ? response.data
                    : locItem;
                }),
              ),
      ...
      moveItem(itemData.location.lid, item.gid, item.lid, item.iid)
        .then(() => {
          ...
          dispatch(
            editLocation({
              ...location,
              item_ids: location.item_ids.filter(
                (item_id) => item_id !== item.iid,
              ),
            }),
          );
          dispatch(
            setLocationItems(
              locationItems.filter((locItem) => locItem.iid !== item.iid),
            ),
          );
            ...
            <Form.Group controlId='location'>
              ...
              <Form.Select
                ...
                {locations.map((loc: Location) => {
                  return (
                    <option value={loc.lid} key={loc.lid}>
                      {loc.name}
                    </option>
                  );
                })}
